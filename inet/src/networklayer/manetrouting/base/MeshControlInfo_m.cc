...
void MeshControlInfo::copy(const MeshControlInfo& other)
{
    ...
    this->vectorAddress_var = (other.vectorAddress_arraysize==0) ? NULL : new ManetAddress[other.vectorAddress_arraysize];
...
void MeshControlInfo::parsimUnpack(cCommBuffer *b)
{
    ...
    if (vectorAddress_arraysize==0) {
        ...
        this->vectorAddress_var = new ManetAddress[vectorAddress_arraysize];
...
{
    ManetAddress *vectorAddress_var2 = (size==0) ? NULL : new ManetAddress[size];
...
ManetAddress& MeshControlInfo::getVectorAddress(unsigned int k)
{
...
void MeshControlInfo::setVectorAddress(unsigned int k, const ManetAddress& vectorAddress)
{
    if (k>=vectorAddress_arraysize) throw cRuntimeError("Array of size %d indexed by %d", vectorAddress_arraysize, k);
