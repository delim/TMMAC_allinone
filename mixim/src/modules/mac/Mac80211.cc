 ...
 * file:        Mac80211.cc
 *
 * author:      David Raguin / Marc L�ｽbbers / Andreas Koepke
 *
...
void Mac80211::sendACKframe(Mac80211Pkt * af)
{
    ...
    tx_total_duration += packetDuration(LENGTH_ACK, br);
...
void Mac80211::sendRTSframe()
{
    ...
    tx_total_duration += packetDuration(LENGTH_RTS, br);
...
void Mac80211::sendCTSframe(Mac80211Pkt * af)
{
    ...
    tx_total_duration += packetDuration(LENGTH_CTS, br);
    ...
    if (!fromUpperLayer.empty()) {
        ...
        if(!contention->isScheduled()) {
            ...
            if(channel.isIdle()) {
                ...
                IDLE when the beacon was generated (actually, it was receiving Host B痴 frame). So what happens now is
                that all nodes which have an arrival during Host B痴 frame AND have completed their post-backoff, will
                ...
                The correct behaviour here is for Host A痴 MAC to note that post-backoff has completed (remainingBackoff
...
void Mac80211::finish() {
    BaseMacLayer::finish();
    recordScalar("#tx_duration", tx_total_duration);
}
